...
const PRICING = {
  ...
  'claude-haiku-4-5': {
    ...
    cacheWrite: 1.00 / 1000000,
  },
  'claude-opus-4-6': {
    input: 15.00 / 1000000,
    output: 75.00 / 1000000,
    cacheRead: 1.50 / 1000000,
  ...
  for (const project of allProjects) {
    ...
    for (const file of files) {
      ...
      for (const [model, tokens] of Object.entries(usage)) {
        if (!totalUsage[model]) {
          totalUsage[model] = { inputTokens: 0, outputTokens: 0, cacheReadInputTokens: 0, cacheCreationInputTokens: 0 };
        }
        if (!projectUsage[project][model]) {
          projectUsage[project][model] = { inputTokens: 0, outputTokens: 0, cacheReadInputTokens: 0, cacheCreationInputTokens: 0 };
        }
