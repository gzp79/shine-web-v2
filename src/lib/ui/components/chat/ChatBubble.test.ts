...
describe('ChatBubble', () => {
    test('renders the message text', () => {
        render(ChatBubble, { props: { content: 'hello world' } });
        expect(screen.getByText('hello world')).toBeInTheDocument();
    });

    test('aligns own messages to the end', () => {
        const { container } = render(ChatBubble, { props: { content: 'mine', align: 'end' } });
        const wrapper = container.querySelector('[data-slot="chat-bubble"]');
        expect(wrapper).toHaveClass('justify-end');
    });

    test('aligns other messages to the start', () => {
        const { container } = render(ChatBubble, { props: { content: 'theirs', align: 'start' } });
        const wrapper = container.querySelector('[data-slot="chat-bubble"]');
        expect(wrapper).toHaveClass('justify-start');
    });

    test('renders a centered note without a bubble', () => {
        const { container } = render(ChatBubble, { props: { content: '…', align: 'center' } });
    ...
    test('renders the author label when provided', () => {
        render(ChatBubble, { props: { content: 'hi', author: 'user-1234' } });
